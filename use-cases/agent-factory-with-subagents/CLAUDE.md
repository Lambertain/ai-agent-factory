...
**Ключове правило:** Управління задачами ТІЛЬКИ через Archon (TodoWrite заборонено!)
...
| 2 | `02_workflow_rules.md` | 211 | Workflow та пріоритизація | **ЗАВЖДИ** перед задачею |
| 3 | `03_task_management.md` | 165 | Управління через Archon | При створенні задач |

### 🔧 Додаткові правила (читай за потребою):

| # | Файл | Рядків | Що міститься | Коли читати |
|---|------|--------|--------------|-------------|
| 4 | `04_quality_standards.md` | 146 | Універсальність, модульність | Перед реалізацією |
...
2. **Використовувати TodoWrite** - тільки Archon для задач
3. **Завершувати без рефлексії** - завжди знаходь недоліки та покращуй
...
2. **Універсальність агентів** - 0% проект-специфічного коду
3. **Модульність** - файли до 500 рядків
4. **Трьохетапне виконання** - виконання → перевірка → покращення
5. **Централізована пріоритизація** - через проджект-менеджера
