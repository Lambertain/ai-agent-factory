# ⚠️ ПРАВИЛА ЦЕ USE-CASE ПЕРЕВИЗНАЧАЮТЬ ЗАГАЛЬНІ ПРАВИЛА

**КРИТИЧНО ВАЖЛИВО:**

Цей use-case (agent-factory-with-subagents) використовує **СПЕЦИФІЧНИЙ WORKFLOW**, який **ПЕРЕВИЗНАЧАЄ** загальні правила з `agent-factory\CLAUDE.md`.

## 🔄 Ключові відмінності від загальних правил:

1. **Обов'язкове переключення в роль** - ЖОД НА ЗАДАЧА не виконується як Claude Code
2. **Archon MCP Server** - замість TASK.md для управління задачами
3. **TodoWrite для мікрозадач** - двохрівнева система трекінгу
4. **Модульна архітектура знань** - `knowledge.md + module_selection.md + modules/` замість монолітного `prompts.py`

**Якщо є конфлікт між правилами:**
- ✅ **ВИКОРИСТОВУЙ** правила з ЦЕ use-case (agent-factory-with-subagents)
- ❌ **ІГНОРУЙ** загальні правила з `agent-factory\CLAUDE.md`
...
| 12 | **`11_reflection_confirmation_protocol.md`** | ~450 | **🚨 ПРОТОКОЛ РЕФЛЕКСІЇ ТА ПІДТВЕРДЖЕННЯ** - Single Source of Truth для блокуючих правил | **ЗАВЖДИ** перед застосуванням покращень |
| 13 | **`12_mcp_usage_protocol.md`** | ~200 | **🔌 Протокол використання MCP серверів** | При роботі з MCP (Archon, GitHub і т.д.) |
| 14 | **`13_port_management.md`** | ~200 | **🚨 Керування портами розробки** - централізований реєстр, заборона taskkill | **ЗАВЖДИ** перед запуском dev server |
| 15 | `14_rules_writing_guide.md` | 516 | Посібник з написання правил для агентів | При створенні нових модулів правил |
| 16 | **`15_blueprint_architect_rules.md`** | ~450 | **🏗️ ПРАВИЛА ДЛЯ BLUEPRINT ARCHITECT** - створення модульних правил | **ЗАВЖДИ** при роботі з правилами агентів |
...
15. **🚨 ПИСАТИ "ПРОСТИНІ" З ТЕКСТУ** - тільки суть, стисло, лаконічно. Деталі ТІЛЬКИ якщо користувач явно попросить
...
```
❌ ЗАБОРОНЕНО - "простиня" тексту:
"Розпочинаю роботу над задачею. Спочатку я проаналізую існуючий код,
потім створю детальний план з 15 пунктів, після чого проведу дослідження
всіх можливих підходів, розгляну переваги і недоліки кожного варіанту,
зважу всі ризики, створю матрицю прийняття рішень, після чого почну
імплементацію з детальним логуванням кожного кроку..."

✅ ПРАВИЛЬНО - коротко і по суті:
"Починаю роботу. Аналізую код → планую → імплементую."
або просто:
"🎭 Переключаюся в роль [Role Name]"

❌ ЗАБОРОНЕНО - детальний звіт без запиту:
"Прочитав 15 файлів: file1.ts (234 рядки, 5 функцій, 3 класи,
експортує APIClient і DataService), file2.ts (567 рядків,
включає компоненти Header, Footer, Sidebar з повною типізацією)..."

✅ ПРАВИЛЬНО - лише суть (деталі за запитом):
"Проаналізував 15 файлів. Знайшов APIClient в file1.ts."
```
...
- ✅ Головний CLAUDE.md: ~250 рядків (85% скорочення!)
...
- [🚨 Протокол аналізу проблем (Навігація)](.claude/rules/09_problem_analysis_protocol.md)
  ...
  - [🆕 09d. Захист від дублювання коду](.claude/rules/09d_anti_duplication_protocol.md)
- [🔄 Протокол освіження](.claude/rules/refresh_protocol.md)
- [✅ Post-Task Checklist](.claude/rules/10_post_task_checklist.md)
- [🚨 Протокол рефлексії та підтвердження](.claude/rules/11_reflection_confirmation_protocol.md)
- [🔌 Протокол використання MCP серверів](.claude/rules/12_mcp_usage_protocol.md)
- [🚨 Керування портами розробки](.claude/rules/13_port_management.md)
- [Посібник з написання правил](.claude/rules/14_rules_writing_guide.md)
- [🏗️ Правила для Blueprint Architect](.claude/rules/15_blueprint_architect_rules.md)
...
- [🏗️ Покращення роботи агентів](.claude/AGENT_IMPROVEMENTS_ARCHITECTURE.md) - 6 критичних проблем і рішення (2025-10-09)
