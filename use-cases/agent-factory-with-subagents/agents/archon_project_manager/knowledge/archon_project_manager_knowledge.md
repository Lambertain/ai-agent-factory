...
## Шаг 1: Использовать встроенные MCP инструменты Claude Code

**У ТЕБЯ УЖЕ ЕСТЬ ПРЯМОЙ ДОСТУП К ARCHON MCP!**

Просто используй функции напрямую:
```python
# Получить список проектов
await mcp__archon__find_projects()

# Получить конкретный проект
await mcp__archon__find_projects(project_id="UUID-проекта")

# Получить задачи проекта
await mcp__archon__find_tasks(project_id="UUID-проекта")
...
**❌ НЕ ДЕЛАЙ:**
- ❌ Не пытайся подключаться через Python скрипты
- ❌ Не ищи пути к archon-mcp-server/build/index.js
- ❌ Не создавай MCP клиенты вручную
- ❌ Не используй asyncio.run() обертки

**✅ ДЕЛАЙ:**
- ✅ Используй встроенные функции mcp__archon__* напрямую
- ✅ Эти функции уже настроены и работают в Claude Code
- ✅ Просто вызывай их как обычные async функции
