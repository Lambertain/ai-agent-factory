...
**🚨 КРИТИЧНО: Читай PROJECTS_REGISTRY.md СНАЧАЛА (98% економія токенів!)**
...
async def determine_active_project() -> str:
    ...
    - PROJECTS_REGISTRY.md: ~200 токенів
    - mcp__archon__find_projects(): ~10,800 токенів
    - Економія: 98% (54x швидше)
    """
...
**🚨 КРИТИЧНО: project_id ОБОВ'ЯЗКОВИЙ ПАРАМЕТР (83% економія токенів!)**
...
# Результат: ~500 токенів (задачі з 1 проекту)
...
# Результат: ~3,000 токенів (задачі з AI Agent Factory, iFruite, PatternShift, UniPark, ProjectFlow, eCademy, Suppler Search, Astro, BroastBean)
# Втрата: 83% оптимізації + плутанина в задачах
...
- Втрата 83% оптимізації токенів (3,000 vs 500 токенів)
...
КРОК 3: Отримати пріоритетні задачі (ПОСЛІДОВНА ПЕРЕВІРКА - економія 67% токенів!)
