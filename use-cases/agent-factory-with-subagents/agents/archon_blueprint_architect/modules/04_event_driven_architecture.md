...
Event-Driven Architecture (EDA) - архитектурный стиль, где изменения состояния системы представлены как события, на которые могут подписываться другие компоненты.
...
### Концепция
Вместо хранения текущего состояния, храним последовательность событий, которые привели к этому состоянию.
...
- **Восстановление состояния в любой момент**

### 2. CQRS
- **Оптимизация чтения и записи раздельно**
- **Различные модели данных для read/write**
...
### 1. Event Design
- События в прошедшем времени (UserCreated, не CreateUser)
- Immutable events
- Богатые события с контекстом

### 2. Projections
- Идемпотентные обработчики
- Обработка в правильном порядке
- Rebuild projections при изменении схемы

### 3. Versioning
- Версионирование событий
- Обратная совместимость
- Event Upcasting при миграциях
