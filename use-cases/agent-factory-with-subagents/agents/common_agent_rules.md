...
TodoWrite([
    ...
    {"content": "🚨 ОБОВ'ЯЗКОВО спросити пользователя: применить улучшения сейчас или создать отдельную задачу? (ЧЕКАТИ ВІДПОВІДІ!)", "status": "pending", "activeForm": "🚨 Спрашиваю пользователя о применении улучшений (БЛОКУЮЧЕ)"},
...
2. **🔴 БЛОКУЮЧЕ: Запит підтвердження** - ОБОВ'ЯЗКОВО після рефлексії спитати користувача про застосування покращень
   ...
   > 📖 **Детальний протокол:** `.claude/rules/11_reflection_confirmation_protocol.md`
