...
**Короткі правила:**
- ✅ **ЗАВЖДИ перевіряй** реєстр портів перед запуском
- ✅ **ВИКОРИСТОВУЙ** тільки виділений порт з реєстру
- ❌ **ЗАБОРОНЕНО** `taskkill /f /im node.exe` (убиває всі проекти!)
- ✅ **ПРАВИЛЬНО** `npx kill-port 3000` (тільки конкретний порт)
