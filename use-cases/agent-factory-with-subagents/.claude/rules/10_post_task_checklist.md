...
### ✅ КРОК 2: Перевірка Git операцій

**Для Production проектів:**
- Прочитай `.claude/rules/05_git_integration.md` → розділ "ОБОВ'ЯЗКОВИЙ PUSH ДЛЯ PRODUCTION ПРОЕКТІВ"
- Перевір чи потрібен push для production проекту
- Якщо потрібен → виконай push

**Файл для читання:** `.claude/rules/05_git_integration.md` (рядки 63-221)
